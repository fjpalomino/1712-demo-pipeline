...
public class Navbar extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//Get with a time variable to prevent cache loading
		...
		if (user!=null)
		{
			if((int)session.getAttribute("role")==1)
				request.getRequestDispatcher("features/navbar/managerNav.html").forward(request, response);
			else if((int)session.getAttribute("role")==2)
				request.getRequestDispatcher("features/navbar/employeeNav.html").forward(request, response);
		}
